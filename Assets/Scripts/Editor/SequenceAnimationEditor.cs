...
public class SequenceAnimationEditor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.LabelField("序列帧加载工具", EditorStyles.boldLabel);
    ...
    private void CreateSequenceAnimation(SequenceAnimation sequenceAnimation)
    {
        ...
        if (files.Length == 0)
        {
            EditorUtility.DisplayDialog("警告", "所选文件夹中没有找到图片文件！", "确定");
            return;
        }
        ...
        {
            EditorUtility.DisplayDialog("错误", "没有成功加载任何图片！请确保图片已正确导入为Sprite。", "确定");
            return;
        }
